...
class Viewer(Messages):
    ...
    def __draw(self):
        ...
        if self.image_buffer is not None:
            ...
            # Печатать процесс выполнения
            if self._out is True:
                stdout = self._data_not_received.format(self.red, datetime.now().strftime(self._format_time), self.end)
            else:
                stdout = 0
            raise SystemExit(stdout)
    ...
    def __keyboard(self, key, x, y):
        ...
        if key == 27 or key == b'\x1b':
            # Печатать процесс выполнения
            if self._out is True:
                stdout = '\n' + self._program_close.format(datetime.now().strftime(self._format_time))
            else:
                stdout = 0
            raise SystemExit(stdout)
    ...
    def resize_frame(self, frame, width, height, out = True):
        ...
        if type(frame) is not np.ndarray or len(frame) is 0 or type(height) is not int or height < 0 or type(width) is \
            ...
            # Вывод сообщения
            if out is True:
                self._inv_args(__class__.__name__, self.resize_frame.__name__)
